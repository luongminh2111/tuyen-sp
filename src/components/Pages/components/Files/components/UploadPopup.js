...
import { uploadFile } from "../actions/FileActionCallapi";

function UploadPopup(props) {
  const { open, handleClose, curProject } = props;
  ...
  const handleUpload = () => {
    ...
    uploadBytesResumable(storageRef, file).then(() => {
      getDownloadURL(ref(storages, `/files/${file.name}`)).then(res => {
        ...
        dispatch(uploadFile(request)).then((res) => {
          if (res?.status === 200 && res?.data?.data) {
            setLoading(false);
            setOpenAlert(true);
            setStatusAlert("success");
            setTextAlert("Upload success!");
            dispatch({type: 'ADD_NEW_FILE', item: res.data.data});
            setTimeout(() => {
              handleClose();
            }, 1500);
